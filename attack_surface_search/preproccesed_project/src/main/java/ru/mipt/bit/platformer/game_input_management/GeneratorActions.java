...
@ Component public class GeneratorActions
{
...
if ( randomNumber < 10 )
{
MoveAction moveAction = new MoveAction ( tank , new Direction ( ButtonCommand . UP ) );
commandQueueHandler . addAction ( moveAction );
}
else if ( randomNumber < 20 )
{
MoveAction moveAction = new MoveAction ( tank , new Direction ( ButtonCommand . LEFT ) );
commandQueueHandler . addAction ( moveAction );
}
else if ( randomNumber < 30 )
{
MoveAction moveAction = new MoveAction ( tank , new Direction ( ButtonCommand . DOWN ) );
commandQueueHandler . addAction ( moveAction );
}
else if ( randomNumber < 40 )
{
MoveAction moveAction = new MoveAction ( tank , new Direction ( ButtonCommand . RIGHT ) );
commandQueueHandler . addAction ( moveAction );
}
else if ( randomNumber == 41 )
{
Direction direction = new Direction ( tank . getRotation ( ) );
GridPoint2 coord = new GridPoint2 ( tank . getCoordinates ( ) . cpy ( ) . x + direction . getVector ( ) . x , tank . getCoordinates ( ) . cpy ( ) . y + direction . getVector ( ) . y );
BulletMoveModel bulletMoveModel = level . putBulletInLevel ( coord , direction );
ShootAction shootAction = new ShootAction ( tank , bulletMoveModel , direction );
