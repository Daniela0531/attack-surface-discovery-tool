...
@ Component public final class GdxGameUtils
{
...
case 1 : @ SuppressWarnings ( "unchecked" ) L layer = ( L ) layers . iterator ( ) . next ( );
return layer;
...
{
return new Rectangle ( ) . setWidth ( region . getRegionWidth ( ) ) . setHeight ( region . getRegionHeight ( ) );
}
...
return new Rectangle ( ) . setX ( tileBottomLeftCornerX ) . setY ( tileBottomLeftCornerY ) . setWidth ( tileWidth ) . setHeight ( tileHeight ) . getCenter ( new Vector2 ( ) );
}
